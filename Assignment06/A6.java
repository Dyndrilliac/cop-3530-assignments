...
public class A6
{
    ...
    public static final double[] LOAD_FACTORS = {      // Sample load factors.
    ...
    private final int[]                    empirFailureCounter = new int[A6.LOAD_FACTORS.length * 2]; // Empirical Counters
    private final int[]                    empirSuccessCounter = new int[A6.LOAD_FACTORS.length * 2];
    private final List<HashTable<Integer>> hashTables          = new ArrayList<HashTable<Integer>>(); // Table List
    private int                            maxValueInserted    = 0;                                   // Limits - These are set by user input!
    private int                            maxValueSearched    = 0;
    private final int[]                    probeLengthFailure  = new int[A6.LOAD_FACTORS.length * 2]; // Probe Length Counters
    ...
    {
        StdOut.printf("%d hash tables have been generated and filled with %d random integers between %d and %d (inclusively).%n" + 
                        "The program will now go through and search each table for %d random integers between %d and %d (inclusively).%n" + 
                        "It keeps track of the probe lengths needed to either find an integer or determine that it is not in the table.%n%n", 
                        this.hashTables.size(), 
                        A6.FILL_SIZE, 
                        1, 
                        this.maxValueInserted, 
                        A6.FIND_SIZE, 
                        1, 
                        this.maxValueSearched);
    }
    ...
    private void printResultsTable(final boolean isProbeLinear)
    {
        ...
        StdOut.println("-----------------------------------------------------------------------------------");
        StdOut.printf("%-8.8s %-12.12s %-12.12s %-12.12s %-12.12s %-12.12s %-12.12s%n", "Load", "E-Success", "Denominator", "T-Success", "E-Failure", "Denominator", "T-Failure");
        StdOut.println("-----------------------------------------------------------------------------------");
        ...
        for ( int i = 0; i < rows; i++ )
        {
            ...
            {
                index = i;
                theoreticalSuccess = ( 1.0 / 2.0 ) * ( 1.0 + ( 1.0 / ( 1.0 - L ) ) );                // (1/2)*(1 + 1/(1-L))
                theoreticalFailure = ( 1.0 / 2.0 ) * ( 1.0 + ( 1.0 / Math.pow(( 1.0 - L ), 2.0) ) ); // (1/2)*(1 + 1/(l-L)^2)
            }
            else
            {
                index = ( i + 9 );
                theoreticalSuccess = ( ( -1.0 * ( Mathematics.logarithm(( 1.0 - L ), 2.0) ) ) / L ); // (-ln(1-L))/L
                theoreticalFailure = ( 1.0 / ( 1.0 - L ) );                                          // 1/(1-L)
            }

            if ( this.empirSuccessCounter[index] != 0 )
            {
                empiricalSuccess = ( ( (double) this.probeLengthSuccess[index] ) / ( (double) this.empirSuccessCounter[index] ) );
            }

            if ( this.empirFailureCounter[index] != 0 )
            {
                empiricalFailure = ( ( (double) this.probeLengthFailure[index] ) / ( (double) this.empirFailureCounter[index] ) );
            }

            double[] output = { A6.LOAD_FACTORS[i], empiricalSuccess, this.empirSuccessCounter[index], theoreticalSuccess, empiricalFailure, this.empirFailureCounter[index], theoreticalFailure };

            StdOut.printf("%-8.2f %-12.4f %-12.0f %-12.4f %-12.4f %-12.0f %-12.4f%n", output[0], output[1], output[2], output[3], output[4], output[5], output[6]);
        }
    ...
    public void promptInput()
    {
        ...
        do
        {
            StdOut.printf("Provide two integers on one line separated by a space:%n%n");
